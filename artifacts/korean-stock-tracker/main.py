...
async def search_ticker(q: str = Query(..., min_length=1)):
    ...
    for ticker, name in ticker_cache.items():
        if q in name or q_lower in ticker.lower():
...
async def scan():
    try:
        ...
        results = await asyncio.wait_for(scan_market(max_results=30), timeout=120.0)
...
async def analyze_ticker(ticker: str):
    ...
    try:
        ...
        from claude_analyst import analyze_stock
...
async def chat(req: ChatRequest):
    ...
    if req.ticker:
        from startup_event import ticker_cache
        context = {"ticker": req.ticker, "name": ticker_cache.get(str(req.ticker).zfill(6), req.ticker)}
...
_settings: dict = {
    ...
    "whale_small_threshold": 1_0000_0000,
    "whale_medium_threshold": 5_0000_0000,
    "whale_large_threshold": 20_0000_0000,
}
